    ...
    public function epay_check_sign($data, $mode, $timestamp){
        if(!empty($data)){
            ...
            if (preg_match("/true$/i",$responseTxt) && $isSign) {
                ...
                if (!$order) {
                    if($mode == "return"){
                        header("location: " . url('/order' . $order['id'])); die;
...
//                    header("location: " . url('/order')); die;
    ...
    public function order() {
        ...
        if($pay_type == 'codepay'){ //码支付验签
            ...
            try{
                ...
                $hash_info = db::name('order')->where(['hash' => $hash])->find();
                ...
                /*if($type == 'erc'){
                    $check_sign = $this->checkErcStatus($address,$usdtpay_info['erc_pay_address'],$amount,$hash);
                }elseif($type == 'trc'){
                    $check_sign = $this->checkTrcStatus($address,$usdtpay_info['trc_pay_address'],$amount,$hash);
                }elseif($type == 'bsc'){
                    $check_sign = $this->checkBscStatus($address,$usdtpay_info['bsc_pay_address'],$amount,$hash);
                }elseif($type == 'okc'){
                    $check_sign = $this->checkOkcStatus($address,$usdtpay_info['bsc_pay_address'],$amount,$hash);
                }*/
        ...
        if($check_sign){ //验签成功
            ...
            $updata = ['status' => 'daifahuo', 'paytime' => $timestamp,'hash' => $hash];
    ...
    private function checkTrcStatus($address,$pay_address,$amount,$hash){
        $times = time() + 20;
            ...
            if(isset($datas['transactions'])){
                ...
                foreach ($datas['transactions'] as $data){
                    ...
                    if($data['txID'] == $hash){
                        ...
                        $type = substr($parameter,0,$strlen - 128);
                        ...
                        if($data['ret'][0]['contractRet'] == 'SUCCESS' && $type == 'a9059cbb' && $contract == 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t' && $from == $address && $to == $pay_address && $val == $amount){
    ...
    private function checkErcStatus($address,$pay_address,$amount,$hash){
        
        $url = 'https://mainnet.infura.io/v3/15e5fd7f37c04a6ea18633c2f09a7d3f';
        $timestamp = time();
        ...
        while(time() < $timestamp + 60){
            ...
            if($result['result']['status'] == '0x1'){
                ...
                if(isset($result['result']) && !empty($result['result'])){
                    ...
                    $from = $result['from'];
                    $to = '0x' . substr($result['input'],-104,40);
                    $val = hexdec(substr($result['input'],-64)) / 1000000;
                    $strlen = strlen($result['input']);
                    $remark = substr($result['input'],0,$strlen - 128);
                    $contract = $result['to'];

                    if($remark == '0xa9059cbb' && $contract == '0xdac17f958d2ee523a2206206994597c13d831ec7' && $from == $address && $to == strtolower($pay_address) && $val == $amount){
            ...
            }elseif($result['result']['status'] == '0x0'){
                return false;
            }
    ...
    private function checkBscStatus($address,$pay_address,$amount,$hash){
        ...
        while(time() < $timestamp + 60){
            ...
            if($result['result']['status'] == '0x1'){
                ...
                if(isset($result['result']) && !empty($result['result'])){
                    ...
                    $from = $result['from'];
                    $to = '0x' . substr($result['input'],-104,40);
                    $val = hexdec(substr($result['input'],-64)) / 1000000000000000000;
                    $strlen = strlen($result['input']);
                    $remark = substr($result['input'],0,$strlen - 128);
                    $contract = $result['to'];

                    if($remark == '0xa9059cbb' && $contract == '0x55d398326f99059ff775485246999027b3197955' && $from == strtolower($address) && $to == strtolower($pay_address) && $val == $amount){
    ...
    private function checkOkcStatus($address,$pay_address,$amount,$hash){
        ...
        while(time() < $timestamp + 60){
            ...
            if($result['result']['status'] == '0x1'){
                ...
                if(isset($result['result']) && !empty($result['result'])){
                    ...
                    if($remark == '0xa9059cbb' && $contract == '0x382bb369d343125bfb2117af9c149795c6c65c50' && $from == strtolower($address) && $to == strtolower($pay_address) && $val == $amount){
